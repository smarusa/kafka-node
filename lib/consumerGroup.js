...
ConsumerGroup.prototype._checkTopicPartitionChange = function (callback) {
  this.client.loadMetadataForTopics(this.topics, (error, metadataResponse) => {
    ...
    logger.debug(`Topic/Partitions metadataResponse:\n${util.inspect(metadataResponse)}`);
    const metadata = mapTopicToPartitions(metadataResponse[1].metadata);
    logger.debug(`Topic/Partitions metadata:\n${util.inspect(metadata)}`);
    logger.debug(`Consumer previous topicPartitionLength:\n${util.inspect(this.topicPartitionLength)}`);
    const topicOrPartitionsChanged = _.some(this.topicPartitionLength, function (numberOfPartitions, topic) {
      logger.debug(`Topic/Partitions changed?:\nold: ${util.inspect(numberOfPartitions)}\nnew: ${_.get(metadata, `['${topic}'].length`, 0)}`);
